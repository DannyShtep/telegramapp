...
interface RoomState {
  ...
  countdown_end_time: string | null // Нов��е поле для точного таймера
...
export default async function HomePage() {
  ...
  try {
    ...
    console.error("Server Component initial data fetch error:", e)
    error = e.message
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-white p-4">
        <Card className="bg-gray-900 border-gray-700 p-6 text-center">
          ...
          <p className="text-gray-500 text-sm mt-2">Пожалуйста, попробуйте перезагрузить страницу.</p>
        </Card>
      </div>
    )
  }
...
function RouletteGameClientComponent({
  initialRoomState,
  ...
  const handleError = useCallback(
    (error: string, context: string) => {
      ...
      setCountdownError(error)
      setIsLoading(false)
      hapticFeedback.notification("error")
      setTimeout(() => setCountdownError(null), 5000)
    },
    [hapticFeedback],
  )
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-black text-white relative overflow-hidden touch-manipulation">
      ...
      {countdownError && (
        <div className="fixed top-4 left-4 right-4 z-50">
          <Alert className="bg-red-900/90 border-red-700 backdrop-blur-sm">
            ...
            <AlertDescription className="text-white">{countdownError}</AlertDescription>
          </Alert>
        </div>
      )}
      ...
      <div className="flex justify-center mb-8 relative z-10">
        <div className="border border-gray-600 px-6 py-3 rounded-xl font-medium text-lg bg-black/20 backdrop-blur-sm">
          {formatGiftsText(roomState?.total_gifts || 0)} | {(roomState?.total_ton || 0).toFixed(1)} ТОН
        </div>
      </div>
